<!-- Keep a Changelog guide -> https://keepachangelog.com -->

# OneMonokai Changelog

## [Unreleased]

## [0.2.0] - 2025-05-03

- File status color contrast for version control looks more pleasant now
- Adjust version compatibility with new 2025 build
- Diff preview contrast is more in touch with the theme now
- Inactive background for windows and other elements does not reflect the theme

## [0.1.2] - 2025-01-06

- build: update dependencies to the latest version
- feat: update the maximum compatibility version to the latest build version

## [0.1.1] - 2024-10-18

### Added

- Initial plugin creation.

[Unreleased]: https://github.com/tavanuka/OneMonokai/compare/v0.2.0...HEAD
[0.2.0]: https://github.com/tavanuka/OneMonokai/compare/v0.1.2...v0.2.0
[0.1.2]: https://github.com/tavanuka/OneMonokai/compare/v0.1.1...v0.1.2
[0.1.1]: https://github.com/tavanuka/OneMonokai/commits/v0.1.1
